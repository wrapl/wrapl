...
typedef Math$Matrix$t matrix_t;
...
GLOBAL_FUNCTION(New, 2) {
    ...
    uint32_t NoOfRows = ((Std$Integer$smallt *)Args[0].Val)->Value;
    uint32_t NoOfCols = ((Std$Integer$smallt *)Args[1].Val)->Value;
    matrix_t *Matrix = Riva$Memory$alloc(sizeof(matrix_t) + NoOfCols * NoOfRows * sizeof(Std$Object$t **));
    Matrix->Type = T;
...
AMETHOD(Std$String$Of, TYP, T) {
	...
	Std$String$t *Final = Std$String$add(LeftBracket, LeftBracket);
...
METHOD("+", TYP, T, TYP, T) {
	...
	matrix_t *C = Riva$Memory$alloc(sizeof(matrix_t) + NoOfCols * NoOfRows * sizeof(Std$Object$t **));
	C->Type = T;
...
METHOD("*", TYP, T, TYP, T) {
	...
	for (int I = 0; I < NoOfRows; ++I) {
		for (int J = 0; J < NoOfCols; ++J) {
			Std$Function$result Buffer;
			Std$Object$t **AP = A->Entries + I * Temp;
			Std$Object$t **BP = B->Entries + J;
			Std$Function$call($MUL, 2, &Buffer, *(AP), 0, *(BP), 0);
			Std$Object$t *Value = Buffer.Val;
			for (int K = Temp; --K;) {
...
METHOD("*", TYP, T, TYP, Math$Vector$T) {
	...
	Math$Vector$t *C = Riva$Memory$alloc(sizeof(Math$Vector$t) + NoOfRows * sizeof(Std$Object$t **));
...
static Std$Object$t *determinant2(matrix_t *M, int N, int *Rows, int *Cols) {
	Std$Object$t **Row = M->Entries + Rows[0] * M->NoOfCols.Value;
	++Rows;
...
static inline matrix_t *inverse(matrix_t *M) {
	...
	matrix_t *C = Riva$Memory$alloc(sizeof(matrix_t) + N * N * sizeof(Std$Object$t **));
	C->Type = T;
	C->NoOfRows.Type = C->NoOfCols.Type = Std$Integer$SmallT;
	C->NoOfRows.Value = N;
	C->NoOfCols.Value = N;
	...
	Std$Object$t *Det = determinant2(M, N, Rows, Cols);
...
static inline Std$Function$status sq_mat_pow(Std$Function$result *Result, int N, const matrix_t *A, int Power) {
	if (Power == 1) {
		Result->Val = A;
		return SUCCESS;
	} else {
		matrix_t *R = Riva$Memory$alloc(sizeof(matrix_t) + N * N * sizeof(Std$Object$t **));
		Std$Object$t *One = Std$Integer$new_small(1);
		Std$Object$t *Zero = Std$Integer$new_small(0);
		for (int I = 0; I < N; ++I) {
			for (int J = 0; J < N; ++J) {
		...
		for (;;) {
			if (Power & 1) {
				matrix_t *S = Riva$Memory$alloc(sizeof(matrix_t) + N * N * sizeof(Std$Object$t **));
				sq_mat_mul(N, S, R, P);
				R = S;
			};
			Power >>= 1;
			if (Power == 0) break;
			matrix_t *Q = Riva$Memory$alloc(sizeof(matrix_t) + N * N * sizeof(Std$Object$t **));
			sq_mat_mul(N, Q, P, P);
			P = Q;
		};
		R->Type = T;
...
METHOD("t", TYP, T) {
	...
	uint32_t NoOfRows = A->NoOfRows.Value;
	uint32_t NoOfCols = A->NoOfCols.Value;
	matrix_t *B = Riva$Memory$alloc(sizeof(matrix_t) + NoOfCols * NoOfRows * sizeof(Std$Object$t **));
	B->Type = T;
...
METHOD("+", TYP, T, ANY) {
	...
	C->NoOfRows.Value = NoOfRows;
	C->NoOfCols.Value = NoOfCols;
	memcpy(C->Entries, A->Entries, NoOfRows * NoOfCols * sizeof(Std$Object$t **));
	Std$Object$t **Ptr = C->Entries;
...
METHOD("+", ANY, TYP, T) {
	...
	const Std$Object$t *B = Args[0].Val;
	uint32_t NoOfRows = A->NoOfRows.Value;
	uint32_t NoOfCols = A->NoOfCols.Value;
	matrix_t *C = Riva$Memory$alloc(sizeof(matrix_t) + NoOfCols * NoOfRows * sizeof(Std$Object$t **));
	C->Type = T;
...
METHOD("[]", TYP, T, TYP, Std$Integer$SmallT, TYP, Std$Integer$SmallT) {
    ...
    Std$Integer$smallt *C = Args[2].Val;
...
// Computes the Cholesky decomposition of the symmetric matrix <var>a</var> return a matrix in <var>l</var> and a vector in <var>d</var> such that <code>a = l * diag(d) * l<sup>T</sup></code>.
	...
	matrix_t *L = Riva$Memory$alloc(sizeof(matrix_t) + N * N * sizeof(Std$Object$t **));
	Math$Vector$t *D = Riva$Memory$alloc(sizeof(Math$Vector$t) + N * sizeof(Std$Object$t **));
	L->Type = T;
	...
	Std$Object$t *Zero = Std$Integer$new_small(0);
	Std$Object$t **AP = A->Entries;
	Std$Object$t **LP = L->Entries;
	Std$Object$t **DP = D->Entries;
	Std$Function$result Result0[1];
	for (int I = 0; I < N * N; ++I) LP[I] = Zero;
	for (int I = 0; I < N; ++I) LP[N * I + I] = One;
	for (int I = 0; I < N; ++I) {
		Std$Object$t **DV = DP + I;
		DV[0] = AP[N * I + I];
		for (int K = 0; K < I; ++K) {
