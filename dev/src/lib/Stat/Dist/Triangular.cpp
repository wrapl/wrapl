...
//:T
	...
	if (Arg->Type == Std$Integer$SmallT) {
		Lower = ((Std$Integer$smallt *)Arg)->Value;
	} else if (Arg->Type == Std$Real$T) {
		Lower = ((Std$Real$t *)Arg)->Value;
	};
	Arg = Args[1].Val;
	if (Arg->Type == Std$Integer$SmallT) {
		Mode = ((Std$Integer$smallt *)Arg)->Value;
	} else if (Arg->Type == Std$Real$T) {
		Mode = ((Std$Real$t *)Arg)->Value;
	};
	Arg = Args[2].Val;
	if (Arg->Type == Std$Integer$SmallT) {
		Upper = ((Std$Integer$smallt *)Arg)->Value;
	} else if (Arg->Type == Std$Real$T) {
		Upper = ((Std$Real$t *)Arg)->Value;
	};
