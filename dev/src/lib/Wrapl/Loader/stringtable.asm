...
cfunction stringtable_init;(stringtable Table) -> ()
	mov eax, [esp + 4]
	mov [table(eax).Size], dword 0
	mov [table(eax).Space], dword 0
	ret

cfunction stringtable_put;(stringtable Table, const char *Key, void *Value) -> ()
	push ebx
	push edi
	push esi
	push ebp
...
cfunction stringtable_get;(stringtable Table, const char *Key) -> (void *)
	push ebx
	push edi
	push esi
	push ebp
